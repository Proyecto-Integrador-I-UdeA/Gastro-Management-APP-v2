...
import { businessPermissions } from '../../prisma/permissionConfig';
...
describe('migración de permisos de negocio para admin', () => {
  it('es aditiva, idempotente y no modifica otros roles', async () => {
    await prisma.$transaction(async transaction => {
      ...
      const futurePlatformPermission = 'platform.tenants.manage';
      await transaction.$executeRaw`
        INSERT INTO "permissions" ("name") VALUES (${futurePlatformPermission})
      `;
      await transaction.$executeRaw`
      ...
      expect(adminPermissions.map(permission => permission.name)).toEqual(
        businessPermissions.map(permission => permission.name).sort(),
      );
      expect(adminPermissions.map(permission => permission.name)).not.toContain(
